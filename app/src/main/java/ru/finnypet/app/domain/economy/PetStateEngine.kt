...
class PetStateEngine(private val balance: GameBalance) {
    ...
    /**
     * Цена самой дешёвой покупки, которая закрывает хоть одну текущую
     * потребность (R12): по ней решают, вести ли в магазин и переспрашивать
     * ли перед сном (Б6) — весь набор [cheapestCover] для этого не нужен,
     * закрыть можно и по одной покупке. `null` — потребностей нет или ни
     * один товар ни одну из них не поднимает.
     */
    fun cheapestNeeded(state: PetState, shop: List<ShopItem>): Coins? =
