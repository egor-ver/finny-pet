...
class PlanAdviceTest {
    ...
    /**
     * Б15: ребёнок начал с копилки, до нужного ещё не дошёл — сова не грустит
     * раньше времени, но и не молчит: план уже можно подтвердить, и о нехватке
     * на еду и уход она предупреждает сразу, только спокойно.
     */
    @Test
    fun `копилка тронута раньше нужного — сова спокойно предупреждает`() {
        assertEquals(
            PlanOwl(PetMood.CALM, Explanation("owl.plan.not_enough", mapOf("need" to "37"))),
            owl(plan(0, 0, 8)),
        )
    }

    /** Раскладывать больше нечего, а нужное так и осталось нулевым — предупреждение уже как обычно, грустью. */
    @Test
    fun `весь кошелёк в копилке, нужное так и не тронуто — сова грустит`() {
        assertEquals(
